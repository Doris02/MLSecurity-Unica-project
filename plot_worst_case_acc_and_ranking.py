...
# (minimum robust accuracy across attacks)
...
# sort epsilon values
worst_df = worst_df.sort_values(["eps_255", "robust_acc_worst"])

# reset the dataframe index to a clean consecutive range
...
print(
    worst_df[
        ["eps_255", "short_model", "robust_acc_worst"]
...
# sort epsilon values
rank_df = rank_df.sort_values(["eps_255", "rank"])
...
print(
    rank_df[
        ["eps_255", "short_model", "robust_acc_worst", "rank"]
    ].to_string(index=False)
)
...
# integer ranks only
plt.yticks(
    range(1, len(rank_df["short_model"].unique()) + 1)
)
